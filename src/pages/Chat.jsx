...
const handleStarter = async (text, action = 'send') => {
  setCreatingChat(true)
  try {
    const chat = await createChat()
    navigate(`/chat/${chat._id}`)
    if (action === 'screenshot') {
      setTimeout(() => inputRef.current?.openFilePicker(), 150)
    } else {
...
return (
  ...
  isMobile ? (
    ...
    allModes.map(style => {
      ...
      {!id ? (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', gap: '16px', padding: '0 20px' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', justifyContent: 'center', maxWidth: '480px', marginTop: '8px' }}>
            {[
              { text: 'They left me on read — help me understand', action: 'send' },
              { text: 'Paste a screenshot of a conversation', action: 'screenshot' },
              { text: "I don't know what to reply", action: 'send' },
              { text: 'Is this a red flag?', action: 'send' }
            ].map(starter => (
              <button
                key={starter.text}
                onClick={() => handleStarter(starter.text, starter.action)}
                disabled={creatingChat}
                style={{
