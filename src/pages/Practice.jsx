...
const Practice = () => {
  ...
  return (
    <div className="practice-layout" style={{ display: 'flex', height: '100vh', backgroundColor: '#0f0f0f', color: 'white', overflow: 'hidden' }}>
      ...
      <div className="practice-main" style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px', overflowY: 'auto' }}>
        <div style={{ width: '100%', maxWidth: '520px', display: 'flex', flexDirection: 'column', gap: '28px' }}>
          ...
          {!isPro && (
            <div style={{ padding: '12px 16px', backgroundColor: 'rgba(168,85,247,0.08)', border: '1px solid rgba(168,85,247,0.15)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap' }}>
              <p style={{ fontSize: '12px', color: '#9ca3af', lineHeight: 1.5 }}>
                <span style={{ color: '#a855f7', fontWeight: '600' }}>Free Plan</span> · {freePracticeLimit} practice sessions included
              </p>
              <button
                onClick={() => navigate('/pricing')}
                style={{ padding: '6px 12px', backgroundColor: '#9333ea', border: 'none', borderRadius: '6px', color: 'white', fontSize: '11px', fontWeight: '600', cursor: 'pointer' }}
              >
                Upgrade ✨
              </button>
            </div>
